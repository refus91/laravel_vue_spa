<template>
    <div class="container flex justify-center">
        <div class="lg:w-2/6 md:w-1/2 bg-gray-100 rounded-lg p-8 flex flex-col w-full mt-10">
            <h2 class="text-gray-900 text-2lg font-medium title-font mb-5">Регистрация<router-link :to="{ name: 'login'}"  class="text-blue-600 hover:text-blue-700 ml-4">Назад</router-link></h2>
            ...
            <div class="relative mb-4">
                ...
                <input v-model="c_password" type="password" id="c_password" name="c_password" class="w-full bg-white rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
            </div>
            <button :disabled="!isDisabled" @click.prevent="register" type="submit" value="register" class="text-white bg-indigo-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg disabled:opacity-50">Подтвердить</button>
        </div>
    </div>
</template>
    ...
    methods: {
        register(){
            axios.get('/sanctum/csrf-cookie').then(response => {
                axios.post('/api/register',{ name: this.email, email: this.email, password: this.password, c_password: this.c_password })
                    .then( res => {
